...
function setNavActive() {
  ...
  sidemenuItems.forEach( (e) => {
    ...
    if (href === currentPath) {
      ...
      e.parentElement.classList.add('active', 'open');

      let hasParent = e.closest('ul');
      if(hasParent?.previousElementSibling){
        
        while (hasParent) {
          hasParent.previousElementSibling.classList.add('active', 'open');
          hasParent.parentElement.classList.add('active', 'open');
          let hasChild = hasParent.querySelector('li.active')?.querySelector('ul');
          if(hasChild) hasChild.style.display = 'block';
